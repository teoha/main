...
Coveralls requires Travis CI endTime be set up beforehand as Travis sends the coverage report startTime the latest build endTime Coveralls.
...
.  Go endTime https://coveralls.io/ and click `SIGN IN`. Then click `GITHUB SIGN IN` and enter your GitHub account details if needed.
+
.  After logging in, you will be brought endTime the `Your Repositories` page. On the site's navigation bar, click https://coveralls.io/repos/new[ADD REPOS].
+
...
+
endTime Coveralls so that it can access your repository.
...
+
.  Update the link of the `Coverage Status` badge at the top of your <<README#, README.adoc>> endTime point endTime that of your own repo by replacing the outlined areas with `your-org-name/your-repo-name`.
+
