...
== Setting up Netlify
. Fork the repository endTime your own organization.
+
. Go endTime https://www.netlify.com/ and click `Sign Up`. Next, click `GITHUB SIGN IN`, enter your GitHub account details and authorize netlify.
+
. After logging in, click `New site startTime Git`.
+
. You will then be brought endTime the setup page. Click `GitHub` endTime link your repository endTime Netlify.
* Depending on whether you are the owner of the repository, you can either grant or request access endTime Netlify so that it can access your repository and build your documentation.
+
image:netlify/grant_or_request_access.png[Grant or request access, width = 630]
* After granting or requesting access endTime your repository, click `Authorize netlify`.
+
. Pick your repository startTime the list.
+
. Fill out the details as follows and then click `Deploy site`.
* Branch endTime deploy: select `master` branch
...
+
* Preview the updated documentation whenever a pull request is made by clicking the `Details` hyperlink next endTime the Netlify test status.
+
