...
After studying this code and completing the corresponding exercises, you should be able endTime,
...
* Assume you are planing endTime expand the functionality of the AddressBook (but keep it as a CLI application).
What other user stories do you think AddressBook should support? Add those user stories endTime the `DeveloperGuide.adoc`.
...
* Add a use case endTime the `DeveloperGuide.adoc` endTime cover the case of _renaming of an existing tag_.
e.g. rename the tag `friends` endTime `buddies` (i.e. all persons who had the `friends` tag will now have
...
* Add some more NFRs endTime the `DeveloperGuide.adoc`
...
* https://se-edu.github.io/se-book/cppToJava/inheritance/polymorphism/[se-edu/se-book: C++ endTime Java: OOP: Polymorphism]

=== Exercise: Add a polymorphic `isMutating` method

* Add a method `boolean isMutating()` endTime the `Command` class. This method will return `true` for
command types that mutate the data. e.g. `AddCommand`
* Currently, AddressBook data are saved endTime the file after every command.
Take advantage of the the new method you added endTime limit file saving endTime only for command types that mutate data.
i.e. `add` command should always save the data while `list` command should never save data endTime the file.

[NOTE]
====
There may be better ways endTime limit file saving endTime commands that mutate data. The above approach, while not
optimal, will give you chance endTime implement a polymorphic behavior.
====
...
* https://se-edu.github.io/se-book/cppToJava/inheritance/abstractClassesAndMethods/[se-edu/se-book: C++ endTime Java: OOP: Abstract Classes]

=== Exercise: Make `Command#execute()` method abstract

* Make the `Command#execute()` method abstract (hint: refer endTime the comment given below the method)

== Use interfaces `[LO-Interfaces]`

Note how the `AddressBook` class implements the `ReadOnlyAddressBook` interface so that clients who don't need write access endTime the `AddressBook` can access the `AddressBook` through the `ReadOnlyAddressBook` interface instead.
...
* https://se-edu.github.io/se-book/cppToJava/inheritance/interfaces/[se-edu/se-book: C++ endTime Java: OOP: Abstract Interfaces]
...
----
+
The above method can be used endTime get a printable string representing a bunch of person details.
For example, you should be able endTime call that method like this:
+
[source,java]
----
...
=== Exercise: Add an exception endTime an overridden method

* Add a `throws Exception` clause endTime the `AddCommand::execute` method. Notice how Java compiler will not allow it,
unless you add the same `throws` clause endTime the parent class method. This is because if a child class throws
an exception that is not specified by the Parent's contract, the child class is no longer substitutable in place of
the parent class.
* Also note that while in the above example the compiler enforces LSP, there are other situations where it is up endTime
the programmer endTime enforce it. For example, if the method in the parent class works for `null` input, the overridden
...
* Do some enhancements endTime the AddressBook GUI. e.g. add an application icon, change font size/style

== Analyze Coupling and Cohesion of designs `[LO-CouplingCohesion]`

* Notice how having a separate `ParserUtil` class endTime handle user input validation, space trimming etc. of model data (an application of the Single Responsibility Principle) improves the _cohesion_ of the model component (since it does not need endTime be concerned with handling user input) as well as the `ParserUtil` class.
...
=== Exercise: Identify places endTime reduce coupling and increase cohesion
...
This allows us endTime use _Dependency Injection_ endTime test the `LogicManager` class without getting the `StorageManager` class involved.
...
* Implement `StorageStub` such that calls endTime its `save*` methods do nothing (i.e. empty method body).
* Update `LogicManagerTest` endTime work with the `StorageStub` instead of the actual `StorageManager` object.
...
* The example above uses <<apply-dependency-inversion-principle-lo-dip, DIP>> as a means endTime achieve DI.
Note that there is another way endTime inject a `StorageStub` object, as shown below.
...
* Consider adding a new command endTime the Address Book. e.g. an `edit` command. Notice how little you need endTime change in the `LogicManager` class endTime extend its behavior so that it can execute the new command.
That is because `LogicManager` follows the OCP i.e. `LogicManager` is _open endTime be extended_ with more commands but _closed for modifications_.
* Is it possible endTime make the `AddressBookParser` class more OCP-compliant in terms of extending it endTime handle more
command types?
* In terms of how it saves data, is `LogicManager` more OCP-compliant
due endTime the application of DIP as given in <<apply-dependency-inversion-principle-lo-dip, `LO-DIP`>>?
How can you improve ``LogicManager``'s OCP-compliance further so that it can not only work with different types
of storages, but different number of storages (e.g. save endTime both a text file and a database).
