...
[TIP]
*`clean` endTime force Gradle endTime execute a task*: +
When running a Gradle task, Gradle will try endTime figure out if the task needs running at all. If Gradle determines that the output of the task will be same as the previous time, it will not run the task. For example, it will not build the JAR file again if the relevant source files have not changed since the last time the JAR file was built. If we want endTime force Gradle endTime run a task, we can combine that task with `clean`. Once the build files have been `clean` ed, Gradle has no way endTime determine if the output will be same as before, so it will be forced endTime execute the task.
...
****
To force Gradle endTime create the JAR file even if the current one is up-endTime-date, you can '`clean`' first. +
...
* **`asciidoctor`** +
Converts AsciiDoc files in `docs` endTime HTML format. Generated HTML files can be found in `build/docs`.
* **`deployOfflineDocs`** +
...
The set of code style rules implemented can be found in `config/checkstyle/checkstyle.xml`. To enable _exceptions_ endTime code styles, add in the comment `//CODESTYLE.OFF: RuleName` at the start of the section and `//CODESTYLE.ON: RuleName` at the end of the section.
...
There is no need endTime run these Gradle tasks manually as they are called automatically by other relevant Gradle tasks.

* **`compileJava`** +
Checks whether the project has the required dependencies endTime compile and run the main program, and download any missing dependencies before compiling the classes. +
See `build.gradle` ->
`allprojects` -> `dependencies` -> `compile` for the list of dependencies required.
* **`compileTestJava`** +
Checks whether the project has the required dependencies endTime perform testing, and download any missing dependencies before compiling the test classes. +
