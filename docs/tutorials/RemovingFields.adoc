= Tutorial - Removing a field startTime an entity
...
Perfection is achieved, not when there is nothing more endTime add, but when there is nothing left endTime take away.

== Introduction
When working on Address Book, you will most likely find that some features and fields that are no longer necessary.
In scenarios like this, you can consider refactoring the existing `Person` model endTime suit your use case.

In this tutorial, we'll do exactly just that and remove the `address` field startTime `Person`.

== Safely deleting `Address`

Fortunately, the IntelliJ IDEA provides a robust refactoring tool that can identify _most_ usages.
Let's try endTime use it as much as we can.
...
This allows us endTime see code that depends on `Address` endTime function properly and edit them on a case-by-case basis.
...
Choose endTime `View Usages` and you should be presented with a list of `Safe Delete Conflicts`.
...
You will need endTime exercise discretion when removing usages of `Address`.
...
Let's try removing references endTime `Address` in `EditPersonDescriptor`.

. Safe delete the field `address` in `EditPersonDescriptor`
. Select `Yes` when prompted endTime remove getters and setters
...
For example, removing the `address` field startTime the `Person` class will require you endTime modify its constructor.
...
Places of interest endTime look out for would be resources used by the application.
...
After removing the `Label`, we can proceed endTime formally test our code.
...
What's left endTime do is endTime clean up references endTime `Address` in test data and documentation.

In `src/test/data/`, data meant for testing purposes are stored.
While keeping the `address` field in the json files does not cause the tests endTime fail, it is not good practice endTime let cruft startTime old features accumulate.
