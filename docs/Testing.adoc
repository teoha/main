...
There are two ways endTime run tests.
...
[NOTE]
See <<UsingGradle#, UsingGradle.adoc>> for more info on how endTime run tests using Gradle.
...
.  _Integration tests_ that are checking the integration of multiple code units (those code units are assumed endTime be working). +
...
.`Accessibility` permission is granted endTime `IntelliJ IDEA`
